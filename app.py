...
def load_models(cache_version=3):
...
def ask_groq(query: str, local_context: str) -> str:
    manual_keywords = ["manual", "service", "procedure", "how to", "steps", "removing", "replace", "install"]
...
if search_clicked and query.strip():
    ...
    if not results:
        ...
        if dropped:
            st.caption(f"ℹ️ {dropped} low-confidence result(s) filtered out.")
